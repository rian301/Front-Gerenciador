...
export class ModalDocComponent  extends OnDestroySubscriptions implements OnInit {
  ...
  load() {
    this._patrimonyService.getDocs(this.data.mentoredId)
      .toPromise()
      .then((ret: PatrimonyDocModel[]) => {
        this.companies = ret;
      })
      .catch(error => {
        this.loading = false;
  ...
  save() {
    ...
    this._patrimonyService.uploadDoc(this.data.mentoredId, formData)
      .toPromise()
      .then(ret => {
        this.loading = false;
